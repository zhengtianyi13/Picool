...
func (service *UserService) Post(ctx context.Context, uId uint, file multipart.File, fileSize int64) serializer.Response {
	...
	userDao := dao.NewUserDao(ctx)        //拿到数据库返回对象，dao是跟数据库进行交互的，dao中是写sql的
	user, err := userDao.GetUserById(uId) //通过dao去获取用户信息，byid
	...
	user.Avatar = path                      //user对象中的Avatar就等于我们现在保存的地址
	err = userDao.UpdateUserById(uId, user) //通过userdao去更新数据库
